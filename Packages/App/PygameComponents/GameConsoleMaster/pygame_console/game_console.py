	...
	def do_script(self, params):
		...
		try:
			...
			with open(params) as f:
				...
				while script_line:  # Execution line by line
...
class Console(pygame.Surface):
	...
	def update(self, events):
		...
		if self.enabled:
			...
			if self.console_input and self.console_input.update(events):
				...
				self.cli.onecmd(self.console_input.get_text())  # onecmd is a super method of python.cmd class, it processes function with "do_" in front of em wtf
	...
	def Render(self, surf, pos=None):  # Overrided render method, called in Packages.App.Application.Render()
